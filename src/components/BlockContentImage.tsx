...
function BlockContentImage({title, shadowed, image}: BlockContentImageProps) {
  ...
  return <div className="group h-full w-full flex items-end justify-start relative bg-cover" style={{backgroundImage: `linear-gradient(0deg, rgba(46,54,77,1) 0%, rgba(46,54,77,0.34) 17%, rgba(46,54,77,0) 33%)`}}>
      {/* <div className={shadowed && "bg-black opacity-15"}></div> */}
      ...
      <div className="h-full w-full relative">
        <div className="absolute inset-0 bg-linear-to-t from-black/70 to-black/0 to-20% group-hover:translate-y-5 transition-all duration-500"></div>
          <img src={image} className="h-full w-full object-cover group-hover:scale-102 transition-transform duration-500"/>
      </div>
      <p className="absolute bottom-6 left-6 text-white text-2xl">{title}</p>
    </div>;
}
